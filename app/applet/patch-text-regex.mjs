...
const prefix = '<span>{roll.isCompleted ? roll.actualLength?.toFixed(1) : roll.targetFormedLength.toFixed(1)} m</span>';
const startIndex = code.indexOf(prefix);
if (startIndex === -1) {
  console.log("NOT FOUND");
  process.exit(1);
}

// Find the line that starts the first block
const blockRegex = /\{roll\.index === 0 && !roll\.isCompleted && lineConfigs\[lineId\]\.fProduced > 0 && \([\s\S]*?\)\}/;
...
const replacement = `{!roll.isCompleted && (() => {
                                         ...
                                         const spillLength = Math.max(0, spillMins * lineConfigs[lineId].speed);
...
const patched = code.replace(match[0], replacement);
